// upload.jsx — Multi-step Upload & Sell flow (responsive)

function UploadPage({ wallet, onWalletRequired }) {
  ...
  const [publishStep, setPublishStep] = React.useState(0);

  const randHex = (n) => Array.from({ length: n }, () => "0123456789abcdef"[Math.floor(Math.random() * 16)]).join("");
  const randBlob = () => "bAE" + Array.from({ length: 32 }, () => "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"[Math.floor(Math.random() * 62)]).join("");
  ...
  const startWalrusUpload = async (f) => {
  ...
  const handlePublish = async () => {
    ...
    const steps = ["Wrapping blob ID…", "Creating Kiosk listing…", "Setting SUI price…", "Confirming via Tatum RPC…", "Finalizing…"];
    for (let i = 0; i < steps.length; i++) {
      setPublishStep(i);
      await new Promise(r => setTimeout(r, 800 + Math.random() * 500));
      setPublishProgress(Math.round(((i + 1) / steps.length) * 100));
    }
  ...
  const reset = () => { setStep(0); setFile(null); setProgress(0); setBlobId(""); setPrice(""); setTitle(""); setDescription(""); setPublishProgress(0); setPublishStep(0); setTxHash(""); setKioskId(""); };

  const publishStepLabels = ["Wrapping blob ID…", "Creating Kiosk listing…", "Setting SUI price…", "Confirming via Tatum RPC…", "Finalizing…"];
  const stepDots = ["Upload", "Processing", "Details", "Publishing", "Live"];
      ...
      <div style={{ display: "flex", alignItems: "center", marginBottom: mobile ? 32 : 44, overflowX: "auto", paddingBottom: 4 }}>
        {stepDots.map((label, i) => (
          <React.Fragment key={i}>
            ...
            {i < stepDots.length - 1 && <div style={{ height: 2, flex: 1, minWidth: mobile ? 12 : 20, background: i < step ? "var(--accent)" : "var(--border)", margin: mobile ? "0 4px" : "0 8px", marginBottom: mobile ? 0 : 22, transition: "background 0.3s" }}/>}
          </React.Fragment>
        ))}
      </div>
      ...
      {step === 0 && (
        <div className="fade-in">
          ...
          <label onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}
            ...
            <input type="file" style={{ display: "none" }} onChange={handleDrop}/>
          </label>
          {!wallet && (
      ...
      {step === 1 && (
        <div className="fade-in" style={{ textAlign: "center" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--t3)" }}>
            <span>Uploading blob…</span><span style={{ color: "var(--accent)", fontWeight: 600 }}>{progress}%</span>
          </div>
          <div style={{ marginTop: 24, background: "var(--s1)", border: "1px solid var(--border)", borderRadius: 10, padding: 14, textAlign: "left" }}>
            {[["Endpoint", "aggregator.walrus-testnet.walrus.space"], ["Redundancy", "2-of-N erasure coding"], ["Epochs", "5 (≈ 5 weeks)"]].map(([k, v]) => (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              ...
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Give your file a clear title"
                style={{ width: "100%", background: "var(--s2)", border: "1px solid var(--border)", borderRadius: "var(--card-radius)", padding: "10px 13px", color: "var(--t1)", fontSize: 14, fontFamily: "inherit", outline: "none" }}/>
            </div>
            <div>
            ...
            {price && (
              <div style={{ background: "var(--s2)", borderRadius: 10, padding: 14, fontSize: 13, color: "var(--t2)" }}>
                {[["Listing price", `${price} SUI`, false], ["Marketplace fee (2.5%)", `−${(parseFloat(price) * 0.025).toFixed(3)} SUI`, false], ["You receive", `${(parseFloat(price) * 0.975).toFixed(3)} SUI`, true]].map(([k, v, accent]) => (
      ...
      {step === 3 && (
        <div className="fade-in" style={{ textAlign: "center" }}>
          ...
          <div style={{ background: "var(--s1)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
            {publishStepLabels.map((label, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 16px", borderBottom: i < publishStepLabels.length - 1 ? "1px solid var(--border)" : "none", background: i === publishStep ? "rgba(0,229,160,0.04)" : "none" }}>
      ...
      {step === 4 && (
        <div className="fade-in" style={{ textAlign: "center" }}>
          ...
          <div style={{ background: "var(--s1)", border: "1px solid rgba(0,229,160,0.2)", borderRadius: 14, padding: 18, textAlign: "left", marginBottom: 20 }}>
            {[["Blob ID", blobId.slice(0, 20) + "…", true], ["Kiosk ID", kioskId.slice(0, 14) + "…"], ["Transaction", txHash.slice(0, 8) + "…" + txHash.slice(-6)], ["Price", `${price} SUI`], ["Status", "Active ✓"]].map(([k, v, accent]) => (
