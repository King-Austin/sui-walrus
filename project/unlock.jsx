// unlock.jsx — Post-purchase file unlock screen (responsive)

function UnlockPage({ file, txHash, wallet, onBack }) {
    ...
    <div className="fade-in" style={{ maxWidth: 560, margin: "0 auto", padding: pad, paddingBottom: mobile ? 88 : 56, textAlign: "center" }}>
      ...
      {phase === "verifying" && (
        <>
          ...
          {/* Terminal */}
          <div style={{ background: "#050810", border: "1px solid var(--border)", borderRadius: 12, padding: mobile ? "14px" : "18px", textAlign: "left", marginBottom: 20, position: "relative", overflow: "hidden" }}>
            ...
            {checkLabels.map((label, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0", fontFamily: "var(--mono)", fontSize: mobile ? 11 : 12 }}>
                <span style={{ color: checks[i] ? "var(--accent)" : i === checks.filter(Boolean).length ? "var(--warn)" : "var(--t3)", flexShrink: 0 }}>
                  {checks[i] ? "✓" : i === checks.filter(Boolean).length ? "›" : "·"}
                </span>
                <span style={{ color: checks[i] ? "var(--t2)" : i === checks.filter(Boolean).length ? "var(--t1)" : "var(--t3)" }}>{label}</span>
                {i === checks.filter(Boolean).length && !checks[i] && (
      ...
      {phase === "ready" && (
        <div className="fade-in">
          ...
          <div style={{ background: "var(--s1)", border: "1px solid rgba(0,229,160,0.2)", borderRadius: 12, padding: 16, textAlign: "left", marginBottom: 24 }}>
            {[["Walrus Blob", blobShort, true], ["Kiosk Object", file?.kiosk?.slice(0, 14) + "…"], ["Verified by", "Tatum RPC · Sui Mainnet"]].map(([k, v, accent]) => (
      ...
      {phase === "done" && (
        <div className="fade-in">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
            {[["TX", txHash ? txHash.slice(0, 8) + "…" : "—"], ["Type", (file?.type || "—").toUpperCase()], ["Size", file?.size || "—"], ["Network", "Sui Mainnet"]].map(([k, v]) => (
