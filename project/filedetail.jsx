// filedetail.jsx — File detail page with purchase flow

function PurchaseModal({ file, wallet, onClose, onSuccess }) {
      ...
      <div className="fade-in" style={{ background: "var(--s1)", border: "1px solid var(--border)", borderRadius: "18px 18px 0 0", padding: "28px 24px 36px", width: "100%", maxWidth: 480, boxShadow: "0 -20px 60px rgba(0,0,0,0.8)" }} onClick={e => e.stopPropagation()}>
        ...
        {phase === "confirm" && (
          <>
            ...
            <div style={{ display: "flex", gap: 12, padding: "12px", background: "var(--s2)", borderRadius: 10, marginBottom: 16, alignItems: "center" }}>
            ...
            <div style={{ background: "var(--s2)", borderRadius: 10, padding: "4px 0", marginBottom: 16 }}>
              {[["Access Pass", "1× Sui Object"], ["Kiosk ID", file.kiosk.slice(0, 10) + "…"], ["Est. Gas", `${gas} SUI`], ["Total", `${file.price} SUI`]].map(([k, v], i) => (
...
function FileDetailPage({ file, wallet, onWalletRequired, onPurchaseSuccess, onBack }) {
  const { mobile } = useViewport();
  const [showPurchase, setShowPurchase] = React.useState(false);
  const [purchased, setPurchased] = React.useState(false);
  if (!file) return null;

  const handleSuccess = (tx) => { setShowPurchase(false); setPurchased(true); onPurchaseSuccess(file, tx); };
      ...
      <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 310px", gap: 20, alignItems: "start" }}>
        ...
        <div style={{ position: mobile ? "static" : "sticky", top: 80 }}>
          <div style={{ background: "var(--s1)", border: "1px solid var(--border)", borderRadius: "var(--card-radius)", padding: 20, marginBottom: 14 }}>
            ...
            {purchased ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px", background: "var(--accent-dim)", border: "1px solid rgba(0,229,160,0.3)", borderRadius: 10, marginBottom: 14, fontSize: 14, fontWeight: 600, color: "var(--accent)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, fontSize: 12 }}>
              {[["🔒", "Access via Sui Kiosk"], ["🌊", "Stored on Walrus"], ["⚡", "Instant on payment"], ["🔍", "Verified · Tatum RPC"]].map(([ic, tx]) => (
...
Object.assign(window, { FileDetailPage });
