// marketplace.jsx — Main marketplace listing page

function FileCard({ file, onClick }) {
  const [hov, setHov] = React.useState(false);
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={() => onClick(file)}
...
function MarketplacePage({ onFileSelect, wallet, onWalletRequired }) {
  ...
  const filtered = React.useMemo(() => {
    ...
    if (search.trim()) files = files.filter(f => f.title.toLowerCase().includes(search.toLowerCase()) || f.tags.some(t => t.toLowerCase().includes(search.toLowerCase())));
  ...
  return (
    <div className="fade-in" style={{ maxWidth: 1280, margin: "0 auto", padding: pad, paddingBottom: mobile ? 80 : pad }}>
      ...
      {/* Category pills */}
      <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "nowrap", overflowX: "auto", paddingBottom: 4, WebkitOverflowScrolling: "touch" }}>
        {categories.map(c => (
